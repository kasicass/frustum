...
sv_mesh_t *sv_mesh_create(float *vertex,int num_vertex) {
	...
	/* оптимизируем вершин?дл?теневого объема */
...
void sv_mesh_render(sv_mesh_t *mesh) {
	glPushMatrix();
	/* применил?матриц?преобразования */
	...
	for(i = 0; i < sv->num_face; i++) {
		if(sv->face[i].plane[0] * light[0] +
		   ...
		   sv->face[i].plane[3] * light[3] > 0) {
			/* если гран?видн?из лампочки
			 * ?идентификатора?ребе?гран?добави?по единичке
			 * если ?итог?идентификато?раве?0 то эт?ребр?- границ?			 * кром?того задади?направлени?обхода */
			sv->face[i].e0->id++;
			sv->face[i].e1->id++;
			sv->face[i].e2->id++;
...
/* рендерин?теневого объема ?стенсиль буфе? */
void sv_mesh_render_to_stencil(sv_mesh_t *mesh,float *light) {
	float imatrix[16],ilight[4];
	glPushMatrix();
	/* применил?матриц?преобразования */
	glMultMatrixf(mesh->matrix);
	if(light[3] == 0) {
		/* если лампочка на бесконечност?то ?ее координата?добави?		 * координаты объект?*/
		v_translate(mesh->matrix,ilight);
		v_add(ilight,light,ilight);
	} else {
		/* лампочка рядо?ее координаты не трогае?*/
		v_copy(light,ilight);
	}
	/* координаты лампочки умножаем на обратную матриц?преобразования */
	m_inverse(mesh->matrix,imatrix);
	v_transform(ilight,imatrix,ilight);
	ilight[3] = light[3];
	/* нашл?границ?*/
	sv_mesh_silhouette(mesh->sv,mesh->matrix,ilight);
	/* рендерим только ?стенсиль */
